...
const initialData = [
    {
        ...
        patient: {
            ...
            gender: 'Male',
            weight: '30kg',
            medicalHistory: ['Vaccinated for Rabies', 'Previous surgery on left leg', 'Allergy to peanuts'],
...
function Nursedashboard() {
    ...
    const handleAddAppointment = () => {
        // Form validation
        if (!newAppointment.patient_id || !newAppointment.doctor_id || !newAppointment.appointment_date || !newAppointment.time_slot || !newAppointment.reason || !newAppointment.status) {
    ...
    const handleAddPatient = () => {
        // Form validation
        if (!newPatient.name || !newPatient.species || !newPatient.breed || !newPatient.age || !newPatient.gender || !newPatient.weight || !newPatient.owner.firstName || !newPatient.owner.lastName || !newPatient.owner.phone || !newPatient.owner.email || !newPatient.owner.address) {
        ...
        <Box sx={{ padding: 4 }}>
            ...
            <Grid container spacing={2} sx={{ marginBottom: '20px' }}>
                <Grid item>
                    ...
                    <Button variant='contained' color='secondary' sx={{ marginLeft: 2 }} onClick={handlePatientModalOpen}>
                        Create Patient
                    </Button>
                </Grid>
            </Grid>
    ...
    <Table>
        ...
        <TableBody>
            {appointments.map((row) => (
                <TableRow key={row.appointment_id} hover onClick={() => handleRowClick(row)}>
                    <TableCell>{row.appointment_id}</TableCell>
                    <TableCell>{new Date(row.appointment_date).toLocaleString()}</TableCell>
                ...
                {selectedRow && (
                    <Box sx={{ width: 500, padding: 3 }}>
                        <AppBar position="static">
                            <Toolbar>
                                <IconButton edge="start" color="inherit" onClick={() => setSelectedRow(null)} aria-label="close">
                                    <CloseIcon />
                                </IconButton>
                        ...
                        <Paper elevation={3} sx={{ padding: 2, marginBottom: 2 }}>
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    <Typography variant='body1'>
                                        <strong>Last Updated:</strong> {selectedRow.patient.lastUpdated}
                                    </Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant='body1'>
                                        <strong>Medical History:</strong> {selectedRow.patient.medicalHistory.join(', ')}
                                    </Typography>
                                </Grid>
                            </Grid>
                        </Paper>
            ...
            <Modal open={modalOpen} onClose={handleModalClose}>
                <Box sx={{ width: 400, margin: 'auto', marginTop: '20px', padding: 4, backgroundColor: 'white', borderRadius: 2, maxHeight: '80vh', overflowY: 'auto' }}>
                    ...
                    <TextField
                        ...
                        onChange={(e) => handleAppointmentChange('appointment_id', e.target.value)}
                        sx={{ marginBottom: 2 }}
                        error={!newAppointment.appointment_id}
                        helperText={!newAppointment.appointment_id ? 'Appointment ID is required' : ''}
                    />
                    <TextField
                        label='Doctor ID'
                        fullWidth
                        ...
                        helperText={!newAppointment.doctor_id ? 'Doctor ID is required' : ''}
                    />
                    <TextField
                        label='Patient ID'
                        fullWidth
                        ...
                        onChange={(e) => handleAppointmentChange('patient_id', e.target.value)}
                        sx={{ marginBottom: 2 }}
                        error={!newAppointment.patient_id}
                        helperText={!newAppointment.patient_id ? 'Patient ID is required' : ''}
                    />
                    <TextField
                        type='datetime-local'
                        fullWidth
                        ...
                        helperText={!newAppointment.appointment_date ? 'Appointment date is required' : ''}
                    />
                    <TextField
                        label='Time Slot'
                        fullWidth
                        ...
                        helperText={!newAppointment.time_slot ? 'Time slot is required' : ''}
                    />
                    <TextField
                        label='Reason'
                        fullWidth
                    ...
                    />
                    <Button variant='contained' onClick={handleAddAppointment} sx={{ marginTop: 2 }}>
                        Add Appointment
                    </Button>
                </Box>
            </Modal>
            ...
            <Modal open={patientModalOpen} onClose={handlePatientModalClose}>
                <Box sx={{ width: 600, margin: 'auto', marginTop: '20px', marginBottom: '20px', padding: 4, backgroundColor: 'white', borderRadius: 2, maxHeight: '80vh', overflowY: 'auto' }}>
                    ...
                    <Grid item xs={12}>
                        <TextField
                            ...
                            onChange={(e) => handlePatientChange('medicalHistory', e.target.value.split('\n'))}
                            sx={{ marginBottom: 2 }}
                            error={!newPatient.medicalHistory.length}
                            helperText={!newPatient.medicalHistory.length ? 'Medical history is required' : ''}
                        />
                    </Grid>
                    <Grid item xs={12}>
                        <TextField
                    ...
                    <Typography variant='h6' component='h5' sx={{ marginBottom: 2, marginTop: 2 }}>
                        Owner Details
                    </Typography>
                    <TextField
                        ...
                        onChange={(e) => handlePatientChange('owner.firstName', e.target.value)}
                        sx={{ marginBottom: 2 }}
                        error={!newPatient.owner.firstName}
                        helperText={!newPatient.owner.firstName ? 'Owner first name is required' : ''}
                    />
                    <TextField
                        ...
                        onChange={(e) => handlePatientChange('owner.lastName', e.target.value)}
                        sx={{ marginBottom: 2 }}
                        error={!newPatient.owner.lastName}
                        helperText={!newPatient.owner.lastName ? 'Owner last name is required' : ''}
                    />
                    <TextField
                        label='Owner Phone'
                        fullWidth
                        ...
                        helperText={!newPatient.owner.phone ? 'Phone number is required' : ''}
                    />
                    <TextField
                        label='Owner Email'
                        fullWidth
                    ...
                    <Button variant='contained' onClick={handleAddPatient} sx={{ marginTop: 2 }}>
                        Add Patient
                    </Button>
                </Box>
            </Modal>
